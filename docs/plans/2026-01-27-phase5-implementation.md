...
**Architecture:** The compiler is a Claude Code skill that parses workflow spec markdown, resolves node references (creating stubs if needed), clarifies ambiguities interactively, and generates TypeScript using the 0pflow SDK. Tools are unified with the Executable interface so `ctx.run()` works for agents, nodes, and tools.

**Tech Stack:** TypeScript, Zod, vitest, gray-matter (markdown parsing), 0pflow SDK
...
Invoke `/0pflow:compile-workflow url-check` and verify:
