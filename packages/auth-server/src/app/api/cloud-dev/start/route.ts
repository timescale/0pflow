...
import { listMachines, startMachine } from "@/lib/fly";

/**
 * POST /api/cloud-dev/start
 * Start a stopped or suspended cloud dev machine.
 */
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Start all stopped/suspended machines for this app
    const machines = await listMachines(flyAppName);
    let started = 0;
    for (const machine of machines) {
      if (machine.state === "stopped" || machine.state === "suspended") {
        await startMachine(flyAppName, machine.id);
        started++;
      }
    }

    return NextResponse.json({
      data: { status: "starting", started },
    });
  } catch (err) {
    return NextResponse.json(
      {
